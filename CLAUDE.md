...
Portée volontairement limitée pour l'instant : seul `inscription_screen.dart` reste un trou — écran 100 % mock (champs saisis ignorés, aucun compte Firebase Auth/Firestore créé, code d'invitation non validé faute de collection `codes_invitation`, voir « Architecture des données » plus haut). Le compte famille de test a été créé manuellement, comme le premier compte pro.
