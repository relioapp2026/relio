...
**Suite du chantier Référentiel** : R2 = modèles Dart `Usager`/`Unite`, service de lecture, et règles de sécurité sur les trois collections (`allow write: if false` — le référentiel se peuple par seed, jamais depuis le client) ; R2 inclut aussi le compte famille à deux `usagersIds` (cas fratrie). R3 = débranchement de `mockUsagersCatalogue` dans les écrans, après comptage des références.
