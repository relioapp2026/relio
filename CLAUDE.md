...
Portée volontairement limitée pour l'instant : le flux famille n'a pas d'écran d'inscription réellement branché (voir « Inscription » plus bas), et aucun compte Firestore `role: "famille"` n'existe encore pour tester ce chemin de bout en bout — seul le chemin `role: "pro"` a été validé sur le Pixel 9a jusqu'ici.
