...
**Deux ponts temporaires assumés, à ne pas confondre :**
...
- Les consentements image (`ConsentImageScreen`, `ConfidentialiteRGPDScreen`) sont toujours lus et écrits dans `mockUsagersCatalogue`, pas dans Firestore — **aucune persistance réelle, y compris après R3a** (c'est le pont temporaire n°1 ci-dessus, levée prévue en R3b). La collection `usagers` porte déjà `consentImage` depuis R1, mais aucune écriture cliente n'y est autorisée. Écart connu, documenté, sans donnée réelle en jeu à ce stade — mais à fermer avant le pilote.
- **Cas de test du badge de consentement — préservés par R3a, à reposer en R3b** : six usagers (`usager_001`, `002`, `003`, `017`, `031`, `032`) portent un `consentImage` délibérément varié (tout accepté / tout refusé / mixte) qui teste les deux états du badge d'alerte. R3a les conserve intacts puisque le consentement reste lu sur le mock — et la couverture s'est même améliorée, `selection_usager_journal_screen` affichant désormais tout le périmètre du pro plutôt que 5 usagers en dur, donc les 6 cas sont visibles avec le compte de Séb. Ils disparaîtront **en R3b**, quand le consentement passera sur Firestore (le seed met tout à `false`). À reposer à ce moment-là **avec `saisiPar` à `null` ou un uid semé, jamais un uid réel** : les fixtures actuelles portent `saisiPar: mockProConnecteUid`, qui contient le vrai uid Firebase de Séb — le graver dans `referentiel.json` (versionné) serait irréversible, l'historique Git ne se réécrit pas.
