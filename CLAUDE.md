...
3. **Composant PublicationCard** : avatar + horodatage, image bord-à-bord au **ratio 4:5** (portrait — la hauteur fixe de 200 px d'origine tronquait sévèrement les photos verticales, qui sont la majorité de ce que produit un téléphone ; changé le 2026-08-19 sur retour d'usage réel), rangée like/commentaire avec compteurs, texte, 2 premiers commentaires affichés, bottom sheet pour tous les commentaires (fermeture par swipe vers le bas). Pas de badge de contexte sur les publications. **Un tap sur une photo ouvre `PhotosPleinEcranScreen`** : image entière (`BoxFit.contain`, jamais recadrée), zoom au pincement (`InteractiveViewer`), défilement entre les photos de la publication, fond noir. Le fil recadre pour garder des cartes régulières, la visionneuse rattrape ce recadrage — les deux sont complémentaires, ne pas aligner l'un sur l'autre. Même composant côté pro et côté famille.
